...
export default function Upload({ benutzer }: { benutzer: string }) {
  ...
  async function handleStart() {
    ...
    const { jobId: jid } = await uploadFiles(files, benutzer);
    setJobId(jid);
  }
  ...
  if (!jobId) {
    return (
      <div>
        <p style={{ marginBottom: "0.5rem", color: t.textSecondary }}>
          Wähle Fotos aus oder ziehe sie hierher.
        </p>
        <div
          onDrop={handleDrop}
          onDragOver={(e) => {
        ...
        >
          ...
          />
          {files.length === 0 ? (
            <p style={{ color: t.textLighter }}>
              Bilder hierher ziehen oder klicken zum Auswählen
            </p>
          ) : (
            <p style={{ color: t.text }}>
              <strong>{files.length}</strong> Dateien ausgewählt
            </p>
          )}
        </div>
        {files.length > 0 && (
  ...
  return (
    <div>
      {s?.running && (
        <div style={{ marginBottom: "1rem" }}>
          ...
          <button
            ...
            style={{ ...btnSecondary(t), marginTop: "0.5rem" }}
          >
            Abbrechen
          </button>
        </div>
      ...
      {s?.apiBlocked && (
        <p style={{ color: t.danger, marginBottom: "0.5rem" }}>
          API-Tageslimit erreicht (429)
        </p>
      )}
        ...
        >
          <thead>
            <tr style={{ borderBottom: `2px solid ${t.border}` }}>
              ...
              <th style={{ textAlign: "left", padding: "0.4rem", width: 50 }}>
                Status
              </th>
              ...
              >
                ...
                <td
                  style={{
                    ...
                    color: r.status === "error" ? t.danger : t.text,
                  }}
                >
                  {r.msg}
                </td>
